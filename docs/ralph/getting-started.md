...
When you're happy with the plan, tell Claude to save it to `.ralph/PRD.md`.
...
| `@.ralph/PRD.md`                | Points Claude at your requirements doc            |
| `@.ralph/progress.txt`          | Tracks completed work between runs                |
...
for ((i=1; i<=$1; i++)); do
  result=$(docker sandbox run claude --permission-mode acceptEdits -p "@.ralph/PRD.md @.ralph/progress.txt \
